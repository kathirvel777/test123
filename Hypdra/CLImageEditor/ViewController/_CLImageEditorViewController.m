...
<CLImageToolProtocol, UINavigationBarDelegate,UIGestureRecognizerDelegate,GADBannerViewDelegate>
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.toolInfo = [CLImageToolInfo toolInfoForToolClass:[self class]];CLImageToolInfo *tool = [self.toolInfo subToolInfoWithToolName:@"CLToneCurveTool" recursive:NO];
        tool.title = @"ToneCurve";
        tool.available = YES;     // if available is set to NO, it is removed from the menu view.
        tool.dockedNumber = 1;
...
- (void)initNavigationBar
{
    ...
    _navigationBar.barTintColor=UIColorFromRGB(0x2d2c65);
    
    _navigationBar.titleTextAttributes=@{
                                         NSFontAttributeName : [UIFont fontWithName:@"FuturaT-Book" size:22.0],
                                         NSForegroundColorAttributeName : [UIColor whiteColor]
                                         };
    
    _navigationBar.tintColor=UIColorFromRGB(0xffffff);
...
- (void)initMenuScrollView
{
    if(self.menuView==nil)
    {
        ...
        {
        menuScroll.top = self.view.height - menuScroll.height-44;
        }
        else{
        ...
        menuScroll.showsHorizontalScrollIndicator = YES;
        menuScroll.showsVerticalScrollIndicator = YES;
        
        [self.view addSubview:menuScroll];
        self.menuView = menuScroll;
    }
    //  self.menuView.backgroundColor = [CLImageEditorTheme toolbarColor];
    self.menuView.backgroundColor = UIColorFromRGB(0x262d32);
  
    if([[[NSUserDefaults standardUserDefaults]valueForKey:@"MemberShipType"] isEqualToString:@"Basic"])
    {
    self.bannerView = [[GADBannerView alloc]initWithAdSize:kGADAdSizeBanner];

    bview=[[UIView alloc]initWithFrame:CGRectMake(0, _menuView.frame.origin.y+_menuView.frame.size.height, self.view.frame.size.width,51)];
...
- (void)initImageScrollView
{
    if(_scrollView==nil)
    {
        ...
        imageScroll.showsVerticalScrollIndicator = YES;
...
{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(finalEffect:) name:@"FinalEffect" object:nil];
...
-(void)finalEffect:(NSNotification *)notification
{
    self.navigationItem.rightBarButtonItem.enabled=YES;
    self.navigationItem.rightBarButtonItem.title=@"Done";
    
    [[NSNotificationCenter defaultCenter]removeObserver:@"FinalEffect"];
    
    NSLog(@"final noti %@",notification.userInfo);
    NSDictionary *finalEffectImg=notification.userInfo;
    
    UIImage *img=[finalEffectImg objectForKey:@"finalEffect"];
...
{
    return [CLImageEditorTheme localizedString:@"CLImageEditor_DefaultTitle" withDefault:@"Image Editor"];
}
...
- (void)refreshToolSettings
{
    ...
    [[NSUserDefaults standardUserDefaults]setBool:YES forKey:@"isSmallIcon"];
    ...
    for(CLImageToolInfo *info in self.toolInfo.sortedSubtools){
       ...
       // view.backgroundColor = [UIColor redColor];
        ...
        x += W+padding;
        //        CGPoint center = CGPointMake(view.center.x, _menuView.bounds.size.height/2);
        //        view.center = center;
    }
...
- (void)refreshImageView
{
    ...
    [[NSUserDefaults standardUserDefaults]setValue:NSStringFromCGRect(_imageView.frame) forKey:@"imgViewFrame"];
...
- (void)swapToolBarWithEditting:(BOOL)editting
{
    ...
    if(self.currentTool){
        ...
        item.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Okay" style:UIBarButtonItemStyleDone target:self action:@selector(pushedDoneBtn:)];
        
        [item.leftBarButtonItem setTitleTextAttributes:@{NSFontAttributeName:[UIFont fontWithName:@"FuturaT-Book" size:22.0],NSForegroundColorAttributeName:[UIColor whiteColor]} forState:UIControlStateNormal];
        ...
        [item.rightBarButtonItem setTitleTextAttributes:@{NSFontAttributeName:[UIFont fontWithName:@"FuturaT-Book" size:16.0],NSForegroundColorAttributeName:[UIColor whiteColor]} forState:UIControlStateNormal];
...
{
    NSString *MYStickerTitle=sender.view.toolInfo.title;
    
    if([MYStickerTitle isEqualToString:@"Action"])
    {
        //  [self initCv];
        UIStoryboard *mainStory=[UIStoryboard storyboardWithName:@"MainUpload" bundle:nil];
        
        ImgEditorCollectionViewController *vc=[mainStory instantiateViewControllerWithIdentifier:@"ImageeditorCollectionView"];
        
        [self.navigationController pushViewController:vc animated:YES];
    }
    
    else if([MYStickerTitle isEqualToString:@"Sticker"])
    {
        NSData *data=UIImagePNGRepresentation(_originalImage);
        [[NSUserDefaults standardUserDefaults]setObject:data forKey:@"imgData"];
        UIStoryboard *mainStory=[UIStoryboard storyboardWithName:@"MainUpload" bundle:nil];
        
        ImgEditorStickerViewController *vc=[mainStory instantiateViewControllerWithIdentifier:@"ImageeditorStickerView"];
...
- (IBAction)pushedDoneBtn:(id)sender
{
    self.navigationItem.rightBarButtonItem.enabled=YES;
    self.navigationItem.rightBarButtonItem.title=@"Done";
    [self.navigationItem.rightBarButtonItem setTitleTextAttributes:@{NSFontAttributeName:[UIFont fontWithName:@"FuturaT-Book" size:22.0],NSForegroundColorAttributeName:[UIColor whiteColor]} forState:UIControlStateNormal];
    
    self.view.userInteractionEnabled = NO;
    
self.navigationController.navigationBar.backgroundColor=UIColorFromRGB(0x2d2c65);
...
- (void)pushedFinishBtn:(id)sender
{
    if(self.targetImageView==nil)
    {
        ...
        NSData *data=UIImagePNGRepresentation(_originalImage);
        [[NSUserDefaults standardUserDefaults]setObject:data forKey:@"imgData"];
        NSDictionary* userInfo = @{@"total": data};
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"imgEditing" object:self userInfo:userInfo];
