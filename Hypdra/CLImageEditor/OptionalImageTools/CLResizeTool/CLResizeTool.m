...
if(UI_USER_INTERFACE_IDIOM()==UIUserInterfaceIdiomPad)
        ...
        _menuScroll = [[UIScrollView alloc] initWithFrame:CGRectMake(120, 0, _menuContainer.width - 170, _menuContainer.height)];
    else
    ...
    CGPoint center = CGPointMake(_switchBtn.center.x, btnPanel.bounds.size.height/2);
    _switchBtn.center = center;
    _switchBtn.iconView.center = CGPointMake(_switchBtn.iconView.center.x, _switchBtn.bounds.size.height/2);
    _switchBtn.tag = 0;
...
//    _resizePanel.backgroundColor = [[CLImageEditorTheme toolbarColor] colorWithAlphaComponent:0.4];
    CGRect sizeOfPanel=self.editor.imageView.superview.frame;
    sizeOfPanel.size.height=sizeOfPanel.size.height-50;
    sizeOfPanel.origin.y=sizeOfPanel.origin.y+20;
    
    _resizePanel = [[_CLResizePanel alloc] initWithFrame:sizeOfPanel originalSize:_originalImage.size tool:self];
    
    _resizePanel.backgroundColor = [UIColor clearColor];
...
- (UIImage*)imageWithString:(NSString*)str
{
    ...
    label.textColor = UIColorFromRGB(0x262d32);
...
{
    CGFloat W = 70;
    CGFloat H = _menuScroll.height/2;
    CGFloat x = 0;
    ...
    [[NSUserDefaults standardUserDefaults]setBool:NO forKey:@"isSmallIcon"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    for(NSNumber *size in sizes){
        CLToolbarMenuItem *view = [CLImageEditorTheme menuItemWithFrame:CGRectMake(x, H/2.5, W, H) target:self action:@selector(tappedResizePanel:) toolInfo:nil];
        view.userInfo = @{@"size":size};
        view.iconImage = [self imageWithString:[NSString stringWithFormat:@"%@", size]];
        view.iconView.center = CGPointMake(view.iconView.center.x, view.bounds.size.height/2);
        [_menuScroll addSubview:view];
        CGPoint center = CGPointMake(view.center.x, _menuScroll.bounds.size.height/2);
        view.center = center;
        x += W;
    }
...
- (void)initInfoPanelWithTool:(CLResizeTool*)tool
{
    ...
    [label setTextColor:UIColorFromRGB(0x262d32)];
    ...
    [_fieldW setTextColor:UIColorFromRGB(0x262d32)];
    ...
    _fieldW.layer.borderColor = [UIColorFromRGB(0x262d32) CGColor];
    ...
    [_fieldH setTextColor:UIColorFromRGB(0x262d32)];
