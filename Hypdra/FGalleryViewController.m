...
//    [animationView setCenter:currentWindow.center];
...
//    }];
    ...
    _toolbar.barTintColor=UIColorFromRGB(0x2d2c65);
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSLog(@"Current Index = %d",self.cIndex);
    
    _currentIndex = self.cIndex;
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    if( self.navigationController ) {
        if (_useThumbnailView) {
            UIBarButtonItem *btn = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"Share", @"") style:UIBarButtonItemStylePlain target:self action:@selector(handleSeeAllTouch:)] ;
            
            [btn setTitleTextAttributes:@{NSFontAttributeName:[UIFont fontWithName:@"FuturaT-Book" size:20.0],NSForegroundColorAttributeName:[UIColor whiteColor]} forState:UIControlStateNormal];
...
- (void)handleSeeAllTouch:(id)sender
{
    ...
    //    UIImage *image=[UIImage imageNamed:[NSString stringWithFormat:@"edit-3-xxl.png"]];
    //    photo.image = image;
    ...
    /*    NSURL *fb = [NSURL URLWithString:@"fb://"];
     ...
     {
     NSLog(@"instagram n't installed");
     
     }
     }
     else
     {
     NSLog(@"fbn't installed");
    ...
    NSString *path = [self.pathArray objectAtIndex:_currentIndex];
    
    NSLog(@"Given Path = %@",path);
