...
- (id)initWithFrame:(CGRect)frame
{
    ...
    float hght = [[NSUserDefaults standardUserDefaults]floatForKey:@"NavHeight"];
	
    
   // NSLog(@"Nav Height = %f",hght);
	...
	self.contentSize = CGSizeMake(frame.size.width, frame.size.height - hght);
	
	// create the image view
	imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, hght, frame.size.width, frame.size.height - hght)];
...
- (void)setFrame:(CGRect)theFrame
{
   ...
   // NSLog(@"setting frame");
	...
	self.contentSize = CGSizeMake(theFrame.size.width * self.zoomScale, (theFrame.size.height - (hght + 10))  * self.zoomScale );
	
	// resize image view and keep it proportional to the current zoom scale
    
	imageView.frame = CGRectMake( imagePoint.x, theFrame.size.height * 15/100, theFrame.size.width,theFrame.size.height - theFrame.size.height * 30/100);
