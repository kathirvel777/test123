...
- (instancetype)init
{
    ...
    if (self)
    {
        ...
        [pMenuButtonSuperView addGestureRecognizer:tapRecognizer];
...
- (void)createMenuButtons
{
    ...
    for (int i = 0; i < self.numberOfMenuItem; i++) {
        ...
        if (self.menuHeight) {
            ...
            menuButtonSize = self.menuHeight = 45;
        }
        ...
        if (self.menuItemsNameArray.count > i)
        {
           ...
           // menuNameLabel.font = [UIFont systemFontOfSize:8];
            menuNameLabel.font=[UIFont fontWithName:@"FuturaT-Book" size:17];
...
//        pMenuButtonSuperView.layer.backgroundColor = [UIColor clearColor].CGColor;
        pMenuButtonSuperView.layer.backgroundColor = [UIColor colorWithRed:(0/255) green:(0/255) blue:(0/255) alpha:0.6].CGColor;
...
- (void)showCircularMenu{
    ...
    if (menuButtonArray.count <= 0) {
        ...
        CGFloat angle = M_PI / (menuButtonArray.count * 1);
        [UIView animateWithDuration:ANIMATION_DURATION delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:
         ^{
            ...
            NSLog(@"Center X = %f",menuButton.center.x);
            NSLog(@"Center Y = %f",menuButton.center.y);
...
-(void)showGridMenu
{
    ...
    if (menuButtonArray.count) {
        ...
        eachMenuWidth = menuButton.frame.size.width/2;
